...
<section class="content-left">
    ...
    <section class="slider">
      <div id="wowslider-container1">
        <div class="ws_images">
          <ul>
            <% for (int i = 0; i < 5; i++) { %>
            <li><a href="playgameUser.jsp?id=<%= listGameHot.get(i).getGameID() %>" target="_self"><img
        ...
        <div class="ws_bullets">
          <div>
            <% for (int i = 0; i < 5; i++) { %>
            <a href="playgameUser.jsp?id=<%= listGameHot.get(i).getGameID() %>" target="_self"
    ...
    <aside class="sidebar">
      ...
      <section class="content-gamehot">
        ...
        <article class="<%=className %>">
          <a href="playgameUser.jsp?id=<%= listGameHot.get(i).getGameID() %>" target="_self"><img
                  src="<%= request.getContextPath() %>/template/games/img/<%= listGameHot.get(i).getGameImage() %> "
  ...
  <section class="theloai-game">
    ...
    <article class="game-theloai-game">
      <a href="playgameUser.jsp?id=<%= item.getGameID() %>">
        <img
...
<aside class="content-right">
  ...
  <section class="game-choinhieunhat">
    ...
    <section class="content-game-choinhieunhat">
      ...
      %>
      <article class="<%= className %>">
        <a href="playgameUser.jsp?id=<%= item.getGameID() %>"><img
                src="<%= request.getContextPath() %>/template/games/img/<%= item.getGameImage() %>"
                alt="<%= item.getGameName() %>"></a>
        <section>
          <a href="playgameUser.jsp?id=<%= item.getGameID() %>"><h5><%= Helper.cutString(item.getGameName(), 10)%>
          </h5></a>
